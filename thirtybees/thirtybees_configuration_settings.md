...
More detail about generating PDFs is provided in {{site.data.thirtybees.thirtybees_urls.thirtybees_generating_pdfs.link}}, but the configuration settings used for the PDFs are described here.
...
| Field | Required? | Description |
|-------|-----------|-----------|
| **destination** | Where the PDF web version should be served so that Prince XML can find it. By default, this is in `../thirtybees_developers-pdf`, which is just one level above where your project is.  |
