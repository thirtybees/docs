...
Instead of creating links to direct pages, use the data reference technique described in {{site.data.thirtybees.thirtybees_urls.thirtybees_hyperlinks.link}}. With this method, the urls.txt file iterates through all the pages in your navigation and formats them into a YAML syntax. 

Then you insert an inline link by referring to that YAML data. For example, the previous hyperlink is {% raw %}`{{site.data.thirtybees.thirtybees_urls.thirtybees_hyperlinks.link}}`{% endraw %}. 
